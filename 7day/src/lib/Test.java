...
public class Test implements ITest{
	...
	public HashMap<String,String> doTest(List<String> nameList){
		...
		for(int i = 0; i<nameList.size(); i++){
			System.out.println("===");
			System.out.println(nameList.get(i) + "가 시험을 치릅니다.");
			System.out.println("===");
			System.out.println("[을미사변 신미양요 변잉양요 갑신정변 운요호사건]이 일어난 순서대로 입력하세요. (입력을 마치면 end를 끝에 입력)>>");
			
			while(!(ans = scanner.next()).equals("end")){
				answer.add(ans);
			}
	...
	private String checkPass(LinkedList<String> answerSheet){
